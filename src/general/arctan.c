...
s32 LbArcTanAngle(s32 x,s32 y)
{
    ...
    if (x < 0)
    {
        ...
        if (y < 0)
        {
            ...
            if (ux < uy) {
                index = (ux << 8)/uy;
                return 2*LbFPMath_PI   - (s32)lbArcTanFactors[index];
            } else {
                index = (uy << 8)/ux;
                return 3*LbFPMath_PI/2 + (s32)lbArcTanFactors[index];
            }
        } else
        {
            uy = y;
            ...
            if (ux < uy) {
                index = (ux << 8)/uy;
                return   LbFPMath_PI   + (s32)lbArcTanFactors[index];
            } else {
                index = (uy << 8)/ux;
                return 3*LbFPMath_PI/2 - (s32)lbArcTanFactors[index];
            }
    ...
    } else
    {
        ...
        if (y < 0)
        {
            ...
            if (ux < uy) {
                index = (ux << 8)/uy;
                return                 (s32)lbArcTanFactors[index];
            } else {
                index = (uy << 8)/ux;
                return LbFPMath_PI/2 - (s32)lbArcTanFactors[index];
            }
        } else
        {
            uy = y;
