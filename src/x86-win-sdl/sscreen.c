...
TbBool LbHwCheckIsModeAvailable(TbScreenMode mode)
{
    ...
    {
        const s32 minD = min(mdinfo->Height, mdinfo->Width);
        if (minD != 0 && minD < lbMinScreenSurfaceDimension) {
...
int LbI_SDL_BlitScaled(SDL_Surface *src, SDL_Surface *dst)
{
    ...
    {   /* denominator of a (source) pixel's fraction part */
        const s32 denom_i = 2 * dst->h;
        const s32 denom_j = 2 * dst->w;

        /* number of whole units in each (source) step */
        const s32 dsrc_i = 2 * src->h / denom_i;
        const s32 dsrc_j = 2 * src->w / denom_j;

        /* numerator of fractional part of each (source) step */
        const s32 dsrc_num_i = (2 * src->h) - dsrc_i * denom_i;
        const s32 dsrc_num_j = (2 * src->w) - dsrc_j * denom_j;

        /* number of whole units in a (source) half-step */
        const s32 halfdsrc_i = src->h / denom_i;
        const s32 halfdsrc_j = src->w / denom_j;

        s32 dst_offset = 0;
        s32 src_offset = halfdsrc_i * src->w+ halfdsrc_j;

        /* start at fractional part of each (source) half-step */
        s32 src_num_i =  src->h- halfdsrc_i * denom_i;
        s32 src_num_j = src->w- halfdsrc_j * denom_j;

        for (s32 i = 0; i != dst->h; ++i) {
            if (src_num_i > denom_i) {
                src_num_i -= denom_i;
                src_offset += src->w;
            }
            for (s32 j = 0; j != dst->w; ++j) {
